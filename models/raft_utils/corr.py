...
class CorrData:
    def __init__(self,
                 corr: th.Tensor,
                 batch_size: int):
        assert isinstance(corr, th.Tensor)  #确保corr是张量类型
    ...
    def _extract_database_indices_from_query_matches(query_values: torch.Tensor, database_values: torch.Tensor) -> th.Tensor:
        ...
        # 存储database_values中的数量
        num_db = database_values.numel()
        num_q = query_values.numel()

        # 将database_values沿着行方向扩展，具有num_q行
        database_values_expanded = database_values.expand(num_q, -1)
        # 进行转置操作，具有num_q列
        query_values_expanded = query_values.expand(num_db, -1).transpose(0, 1)

        # 进行比较，得到 布尔张量compare，compare[i][j]表示database_values_expanded[i][j]是否等于query_values_expanded[i][j]
        compare = torch.eq(database_values_expanded, query_values_expanded)

        indices = torch.arange(num_db, device=device)   # 创建0-num_db-1的一维张量
        indices = indices.expand(num_q, -1)     # 扩展为具有num_q行的二维张量
    ...
    # 接受一个target_indices参数，可以是整数列表或一个张量，用于从当前的corrdata对象中获取下采样后的相关性数据
    def get_downsampled(self, target_indices: Union[List[int], th.Tensor]) -> CorrData:
        ...
        # 根据索引indices_to_select从当前对象中选择相应的数据
